...
def load_plugins(plugin_name):
    path = Path(f"DarkBots/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"DarkBots.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["DarkBots.modules." + plugin_name] = load
    print("Dark has Imported " + plugin_name)


files = glob.glob("DarkBots/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
