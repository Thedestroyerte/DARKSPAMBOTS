...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __ᴅᴀʀᴋ x sᴘᴀᴍ__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"⚡️ᛕIᔕᛕI Ǥᗩᗩᑎᗪ ᗰᗩᖇᑎI ᕼᗩI ᗷᗝᔕᔕ⚡️_\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴅᴀʀᴋ ᴋᴀ ᴇᴋ ɴᴇᴡ ʙᴇᴛᴀ ᴀᴅᴅ ʜᴜᴀ ʜᴀɪ...__")
        target = ""
        ...
        try:
            ...
            await ok.edit("» ᴍᴀᴅᴀʀᴄʜᴏᴅ ᴀʙ ʀᴇᴘʟʏ ᴋᴀʀ ᴜsᴇʀ ᴘᴇ")
            return
        ...
        if str(target) in sudousers:
            ...
            if len(sudousers) > 0:
                ...
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `__ᴅᴀʀᴋ ᴋᴀ ᴇᴋ ɴᴇᴡ ʙᴇᴛᴀ ᴀᴅᴅ ʜᴜᴀ ʜᴀɪ...__`")
            heroku_var["SUDO_USERS"] = newsudo    
    
    elif event.sender_id in SUDO_USERS:
        await event.reply("» ʏᴇ,ʙᴀss ᴛᴇʀᴀ ᴘᴀᴘᴀ ᴅᴀʀᴋ ᴋᴀʀʀ sᴀᴋᴛᴀ ʜᴀɪ.")
